...
public class ProceduralWorld : MonoBehaviour {
	...
	void Update () {
		...
		var p = new Int2 ( Mathf.RoundToInt ((target.position.x - tileSize*0.5f) / tileSize), Mathf.RoundToInt ((target.position.z - tileSize*0.5f) / tileSize) );
		...
		IEnumerator InternalGenerate () {
			...
			for ( var i=0;i<world.prefabs.Length;i++ ) {
				...
				if ( pref.singleFixed ) {
					...
					var ob = Instantiate ( pref.prefab, p, Quaternion.identity ) as GameObject;
					...
					for ( var sx=0; sx < world.subTiles; sx++ ) {
						for ( var sz=0; sz < world.subTiles; sz++ ) {
							...
							var perl = Mathf.Pow (Mathf.PerlinNoise ( (px + pref.perlinOffset.x)*pref.perlinScale, (pz + pref.perlinOffset.y)*pref.perlinScale ), pref.perlinPower );
							
							var density = pref.density * Mathf.Lerp ( 1, perl, pref.perlin) * Mathf.Lerp ( 1, (float)rnd.NextDouble (), pref.random );
							...
							for ( var j=0;j<count;j++) {
								...
								var ob = Instantiate ( pref.prefab, p, RandomYRot () ) as GameObject;
								ob.transform.parent = root;
								//ob.SetActive ( false );
								//objs.Add ( ob );
								counter++;
