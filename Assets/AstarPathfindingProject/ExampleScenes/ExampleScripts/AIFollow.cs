...
public class AIFollow : MonoBehaviour {
	...
	public void OnPathComplete (Path p) {
		...
		for (var i=0;i<path.Length-1;i++) {
			var dist = AstarMath.DistancePointSegmentStrict (path[i],path[i+1],tr.position);
			if (dist < minDist) {
				notCloserHits = 0;
				minDist = dist;
				pathIndex = i+1;
	...
	public IEnumerator WaitToRepath () {
		var timeLeft = repathRate - (Time.time-lastPathSearch);
	...
	public void Update () {
		...
		var currentWaypoint = path[pathIndex];
		...
		var dir = currentWaypoint - tr.position;
	...
	public void OnDrawGizmos () {
		...
		var  rad = pickNextWaypointDistance;
		if (pathIndex == path.Length-1) {
			rad *= targetReached;
		}
		
		var pP = currentWaypoint + rad*new Vector3 (1,0,0);
		for (float i=0;i<2*Math.PI;i+= 0.1F) {
			var cP = currentWaypoint + new Vector3 ((float)Math.Cos (i)*rad,0,(float)Math.Sin(i)*rad);
