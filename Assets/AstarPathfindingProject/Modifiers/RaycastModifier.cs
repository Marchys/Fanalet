...
namespace Pathfinding {
	...
	public class RaycastModifier : MonoModifier {
	...
	#if UNITY_EDITOR
		[MenuItem ("CONTEXT/Seeker/Add Raycast Simplifier Modifier")]
		...
		public override void Apply (Path p, ModifierData source) {
			...
			for (var it=0;it<iterations;it++) {
				...
				if (subdivideEveryIter && it != 0) {
					...
					var preLength = nodes.Count;
					
					for (var j=0;j<preLength-1;j++) {
						nodes.Add (Vector3.zero);
						nodes.Add (Vector3.zero);
					}
					
					for (var j=preLength-1;j > 0;j--) {
		...
		public bool ValidateLine (GraphNode n1, GraphNode n2, Vector3 v1, Vector3 v2) {
			...
			if (useGraphRaycasting && n1 != null && n2 != null) {
				...
				var graph = AstarData.GetGraph (n1);
				var graph2 = AstarData.GetGraph (n2);
