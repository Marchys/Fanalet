...
namespace Pathfinding {
	...
	public class SimpleSmoothModifier : MonoModifier {
	...
	#if UNITY_EDITOR
		[MenuItem ("CONTEXT/Seeker/Add Simple Smooth Modifier")]
		...
		public override ModifierData output {
			get {
				var result = ModifierData.VectorPath;
		...
		public List<Vector3> SmoothSimple (List<Vector3> path) {
			...
			if (uniformLength) {
				...
				for (var i=0;i<path.Count-1;i++) {
					var length = Vector3.Distance (path[i],path[i+1]);
					
					numSegments += Mathf.FloorToInt (length / maxSegmentLength);
				}
				
				var subdivided = ListPool<Vector3>.Claim (numSegments+1);
				...
				for (var i=0;i<path.Count-1;i++) {
					...
					var numSegmentsForSegment = Mathf.FloorToInt ((length + carry) / maxSegmentLength);
					
					var carryOffset = carry/length;
					//float t = 1F / numSegmentsForSegment;
					
					var dir = path[i+1] - path[i];
					
					for (var q=0;q<numSegmentsForSegment;q++) {
						//Debug.Log (q+" "+c+" "+numSegments+" "+length+" "+numSegmentsForSegment);
						subdivided.Add (dir*(Math.Max (0, (float)q/numSegmentsForSegment - carryOffset)) + path[i]);
						c++;
					}
