...
namespace Pathfinding {
	...
	public class GraphUpdateScene : GraphModifier {
		...
		public void ToggleUseWorldSpace () {
			...
			var matrix = useWorldSpace ? transform.localToWorldMatrix : transform.worldToLocalMatrix;
		...
		public Bounds GetBounds () {
			...
			if (points == null || points.Length == 0) {
				...
				var min = matrix.MultiplyPoint3x4(points[0]);
				var max = matrix.MultiplyPoint3x4(points[0]);
				for (var i=0;i<points.Length;i++) {
		...
		public void Apply () {
			...
			if (points == null || points.Length == 0) {
				...
				if (!useWorldSpace) {
					...
					for (var i=0;i<worldPoints.Length;i++) worldPoints[i] = matrix.MultiplyPoint3x4 (points[i]);
				}
