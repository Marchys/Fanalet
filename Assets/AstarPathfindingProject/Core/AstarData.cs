		...
		public void FindGraphTypes () {
			...
			foreach (var type in types) {
				...
				while (baseType != null) {
					...
					if (Equals ( baseType, typeof(NavGraph) )) {
...
#region GraphCreation
		...
		public NavGraph CreateGraph (Type type) {
			var g = Activator.CreateInstance (type) as NavGraph;
			g.active = active;
			return g;
		}
		...
		public void AddGraph (NavGraph graph) {
			...
			if (graphs != null && graphs.Length >= GraphNode.MaxGraphIndex) {
				throw new Exception("Graph Count Limit Reached. You cannot have more than " + GraphNode.MaxGraphIndex +
...
#region GraphUtility
		...
		public NavGraph FindGraphOfType (Type type) {
			...
			if ( graphs != null ) {
				for (var i=0;i<graphs.Length;i++) {
					if (graphs[i] != null && Equals (graphs[i].GetType (), type)) {
						return graphs[i];
					}
				}
			}
