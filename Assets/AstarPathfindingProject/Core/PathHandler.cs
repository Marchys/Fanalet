...
namespace Pathfinding
{
	...
	public class PathHandler {
		...
		public void InitializeNode (GraphNode node) {
			...
			var bucketNumber = ind >> BucketSizeLog2;
			var bucketIndex = ind & BucketIndexMask;
			...
			if (bucketNumber >= nodes.Length) {
				...
				var newNodes = new PathNode[Math.Max (Math.Max (nodes.Length*3 / 2,bucketNumber+1), nodes.Length+2)][];
				for (var i=0;i<nodes.Length;i++) newNodes[i] = nodes[i];
				...
				for (var i=0;i<nodes.Length;i++) newBucketNew[i] = bucketNew[i];
				
				
				var newBucketCreated = new bool[newNodes.Length];
				for (var i=0;i<nodes.Length;i++) newBucketCreated[i] = bucketCreated[i];
			...
			if (nodes[bucketNumber] == null) {
				...
				if (bucketCache.Count > 0) {
					...
					for (var i=0;i<BucketSize;i++) ns[i] = new PathNode ();
				}
			...
			var pn = nodes[bucketNumber][bucketIndex];
			pn.node = node;
		}
		...
		public void ClearPathIDs () {
			...
			for (var i=0;i<nodes.Length;i++) {
				var ns = nodes[i];
				if (nodes[i] != null) for (var j=0;j<BucketSize;j++) ns[j].pathID = 0;
			}
		}
	}
}
