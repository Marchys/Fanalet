...
#region ThreadingMembers
	...
	public static string[] FindTagNames () {
		if (active != null) return active.GetTagNames ();
			...
			var a = FindObjectOfType (typeof (AstarPath)) as AstarPath;
...
#line default
	...
	private void LogPathResults (Path p) {
		...
		var debug = p.DebugString (logPathResults);
	...
	private int ProcessWorkItems (bool force) {
		if (pathQueue.AllReceiversBlocked) {
			if (processingWorkItems) throw new Exception ("Processing work items recursively. Please do not wait for other work items to be completed inside work items. " +
				"If you think this is not caused by any of your scripts, this might be a bug.");
			
			processingWorkItems = true;
			while (workItems.Count > 0) {
				var itm = workItems.Peek ();
...
#region GraphUpdateMethods
	...
	*/
	...
	private void QueueGraphUpdatesInternal () {
		...
		var anyRequiresFloodFill = false;
	...
	private bool ProcessGraphUpdates (bool force) {
		...
		if (graphUpdateQueueAsync.Count != 0) throw new Exception ("Queue should be empty at this stage");
		
		while (graphUpdateQueueRegular.Count > 0) {
			
			var s = graphUpdateQueueRegular.Peek ();
			
			var threading = s.order == GraphUpdateOrder.FloodFill ? GraphUpdateThreading.SeparateThread : s.graph.CanUpdateAsync(s.obj);

			var forceUnityThread = force;
			...
			if (!forceUnityThread && (threading == GraphUpdateThreading.SeparateAndUnityInit)) {
				...
				if (graphUpdateQueueAsync.Count > 0) {
					...
					if (force) throw new Exception ("This should not happen");
	...
	private void ProcessGraphUpdatesAsync (Object _astar) {
		var astar = _astar as AstarPath;
		if (ReferenceEquals (astar, null)) {
			...
			while (graphUpdateQueueAsync.Count > 0) {
				...
				try {
					if (aguo.order == GraphUpdateOrder.GraphUpdate) {
						...
						throw new NotSupportedException ("" + aguo.order);
					}
				} catch (Exception e) {
	...
	public void Awake () {
		...
		threadInfos = new PathThreadInfo[Math.Max(numThreads,1)];
	...
	private void Initialize () {
		...
		for (var i=0;i<astarData.graphs.Length;i++) {			
...
#region ScanMethods
	...
	public void FloodFill () {
		...
		var smallAreasDetected = 0;
		
		var warnAboutAreas = false;
		
		var smallAreaList = ListPool<GraphNode>.Claim();//new List<GraphNode>();
				...
				if (node.Walkable && node.Area == 0) {
					...
					if (area > GraphNode.MaxAreaIndex) {
						if ( smallAreaList.Count > 0 ) {
							var smallOne = smallAreaList[smallAreaList.Count-1];
	...
	public void InitializeNode (GraphNode node) {
		if (!pathQueue.AllReceiversBlocked) throw new Exception ("Trying to initialize a node when it is not safe to initialize any nodes. Must be done during a graph update");
...
#if UNITY_EDITOR
		if (!Application.isPlaying) {
			if (!pathQueue.AllReceiversBlocked) throw new Exception ("Pathfinding running while not playing?");
			return;
		}
#endif
...
Debug.Log ("Scanning... " + progress.description + " - " + (progress.progress*100).ToString ("0") + "%");
	...
	public void ScanLoop (OnScanStatus statusCallback) {
		...
		for (var i=0;i<graphs.Length;i++) {
			...
			var minp = AstarMath.MapToRange (0.1F,0.7F,(float)(i)/(graphs.Length));
			var maxp = AstarMath.MapToRange (0.1F,0.7F,(float)(i+0.95F)/(graphs.Length));
		...
		lastScanTime = (float)(DateTime.UtcNow-startTime).TotalSeconds;//Time.realtimeSinceStartup-startTime;
	...
	public void ApplyLinks () {
		...
		if (astarData.userConnections != null && astarData.userConnections.Length > 0) {
			...
			var root = new GameObject("Links");
			
			var cache = new Dictionary<Int3, GameObject>();
			for (var i=0;i<astarData.userConnections.Length;i++) {
				var conn = astarData.userConnections[i];
				var a = cache.ContainsKey((Int3)conn.p1) ? cache[(Int3)conn.p1] : new GameObject("Link "+i);
				var b = cache.ContainsKey((Int3)conn.p2) ? cache[(Int3)conn.p2] : new GameObject("Link "+i);
				...
				var ln = a.AddComponent<NodeLink>();
				ln.end = b.transform;
			...
			throw new NotSupportedException ("<b>Links have been deprecated</b>. Please use the component <b>NodeLink</b> instead. Create two GameObjects around the points" +
	...
	public static void WaitForPath (Path p) {
		...
		if (active == null)
			throw new Exception ("Pathfinding is not correctly initialized in this scene (yet?). " +
				"AstarPath.active is null.\nDo not call this function in Awake");
		
		if (p == null) throw new ArgumentNullException ("Path must not be null");
...
#region MainThreads
	...
	public void ReturnPaths (bool timeSlice) {
		...
		var p = pathReturnStack.PopAll ();
		
		if(pathReturnPop == null) {
			pathReturnPop = p;
		} else {
			var tail = pathReturnPop;
			while (tail.next != null) tail = tail.next;
			tail.next = p;
		}
		
		//Hard coded limit on 1.0 ms
		var targetTick = timeSlice ? DateTime.UtcNow.Ticks + 1 * 10000 : 0;
	...
	private static void CalculatePathsThreaded (Object _threadInfo) {
		...
		var astar = threadInfo.astar;
		
		try {
			
			//Initialize memory for this thread
			var runData = threadInfo.runData;
			...
			var targetTick = DateTime.UtcNow.Ticks + maxTicks;
			
			while (true) {
				
				//The path we are currently calculating
				var p = astar.pathQueue.Pop();
	...
	private static IEnumerator CalculatePaths (Object _threadInfo) {
		...
		//Max number of ticks before yielding/sleeping
		var maxTicks = (long)(active.maxFrameTime*10000);
		var targetTick = DateTime.UtcNow.Ticks + maxTicks;
	...
	public NNInfo GetNearest (Vector3 position, NNConstraint constraint, GraphNode hint) {
		...
		var minDist = float.PositiveInfinity;//Math.Infinity;
		var nearestNode = new NNInfo ();
		var nearestGraph = -1;
