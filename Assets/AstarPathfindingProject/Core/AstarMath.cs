...
namespace Pathfinding {
	...
	public class AstarMath {
	    ...
	    public static Vector3 NearestPointStrict(Vector3 lineStart, Vector3 lineEnd, Vector3 point)
	    {
			...
			var magn = fullDirection.magnitude;
	        var lineDirection = magn > float.Epsilon ? fullDirection/magn : Vector3.zero;
	        
	        var closestPoint = Vector3.Dot((point-lineStart),lineDirection); //WASTE OF CPU POWER - This is always ONE -- Vector3.Dot(lineDirection,lineDirection);
	    ...
	    public static float DistancePointSegment2 (Vector3 a, Vector3 b, Vector3 p) {
	    	...
	    	var area = Mathf.Abs (bax * (p.z - a.z) - (p.x - a.x) * baz);
	    	
	    	var d = bax*bax+baz*baz;
	    ...
	    public static float DistancePointSegmentStrict (Vector3 a, Vector3 b, Vector3 p) {
	    	...
	    	var nearest = NearestPointStrict (a,b,p);
			return (nearest-p).sqrMagnitude;
	    }
        ...
        /** Squared distance between two points on the XZ plane */
        public static float SqrMagnitudeXY(Vector3 a, Vector3 b)
        {
            var delta = a - b;
		...
		public static Vector3[] ConvexHull (Vector3[] points) {
			...
			lock (hullCache) {
				...
				do {
					...
					var endpoint = 0;
					for (var i=0;i<points.Length;i++) if (endpoint == pointOnHull || !Left (points[pointOnHull],points[endpoint],points[i])) endpoint = i;
	...
	/*
		...
		public static Vector3[] Subdivide (Vector3[] path, int subdivisions) {
			...
			for (var p=0;p<path.Length-1;p++) {
				var step = 1.0F/Mathf.Pow (2,subdivisions);
		...
		public static Vector3 ClosestPointOnTriangle(Vector3 tr0, Vector3 tr1, Vector3 tr2, Vector3 point)
        {
            ...
            var edge0 = tr1 - tr0;
            var edge1 = tr2 - tr0;
            var a00 = edge0.sqrMagnitude;
            var a01 = Vector3.Dot(edge0, edge1);
            var a11 = edge1.sqrMagnitude;
            var b0 = Vector3.Dot(diff, edge0);
            var b1 = Vector3.Dot(diff, edge1);
