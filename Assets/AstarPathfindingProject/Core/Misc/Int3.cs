...
namespace Pathfinding
{
	...
	public struct Int3 {
		...
		public static Int3 operator * (Int3 lhs, Vector3 rhs) {
			lhs.x = (int)Math.Round (lhs.x * rhs.x);
			lhs.y =	(int)Math.Round (lhs.y * rhs.y);
			lhs.z = (int)Math.Round (lhs.z * rhs.z);
		...
		public Int3 NormalizeTo (int newMagn) {
			...
			x = (int)Math.Round (x/magn);
			y = (int)Math.Round (y/magn);
			z = (int)Math.Round (z/magn);
		...
		public float worldMagnitude {
			get {
				...
				return (float)Math.Sqrt (_x*_x+_y*_y+_z*_z)*PrecisionFactor;
		...
		public float safeSqrMagnitude {
			get {
				var _x = x*PrecisionFactor;
				var _y = y*PrecisionFactor;
				var _z = z*PrecisionFactor;
				return _x*_x+_y*_y+_z*_z;
			}
		}
