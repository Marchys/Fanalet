...
namespace Pathfinding {
	public class NodeLink3Node : PointNode {
		...
		public override bool GetPortal (GraphNode other, List<Vector3> left, List<Vector3> right, bool backwards)
		{
			...
			if ( this.connections.Length != 2 ) throw new Exception ("Invalid NodeLink3Node. Expected 2 connections, found " + this.connections.Length);
	...
	public class NodeLink3 : GraphModifier {
			...
			for ( var a=0;a<connectedNode1.GetVertexCount();a++) {
				...
				for ( var b=0;b<connectedNode2.GetVertexCount();b++) {
					...
					if ( Int3.Angle ( (vb2-vb1), (va2-va1) ) > (170.0/360.0f)*Mathf.PI*2 ) {
						...
						t2 = Math.Min ( t2, AstarMath.NearestPointFactor ( va1, va2, vb1 ) );
						t1 = Math.Max ( t1, AstarMath.NearestPointFactor ( va1, va2, vb2 ) );
		...
		void DrawCircle (Vector3 o, float r, int detail, Color col) {
			var prev = new Vector3(Mathf.Cos(0)*r,0,Mathf.Sin(0)*r) + o;
			Gizmos.color = col;
			for (var i=0;i<=detail;i++) {
				var t = (i*Mathf.PI*2f)/detail;
				var c = new Vector3(Mathf.Cos(t)*r,0,Mathf.Sin(t)*r) + o;
