...
\endcode
		...
		public IEnumerator WaitForPath () {
			if (GetState () == PathState.Created) throw new InvalidOperationException ("This path has not been started yet");
		...
		public uint CalculateHScore (GraphNode node) {
			...
			case Heuristic.Manhattan:
				...
				v1 = (uint)((Math.Abs (hTarget.x-p2.x) + Math.Abs (hTarget.y-p2.y) + Math.Abs (hTarget.z-p2.z))*heuristicScale);
				return v1;
			case Heuristic.DiagonalManhattan:
				...
				p.y = Math.Abs (p.y);
				p.z = Math.Abs (p.z);
				var diag = Math.Min (p.x,p.z);
				var diag2 = Math.Max (p.x,p.z);
...
// Ugly way of writing Conditional("!ASTAR_NO_LOGGING")
		...
		private void ErrorCheck () {
			if (!hasBeenReset) throw new Exception ("The path has never been reset. Use pooling API or call Reset() after creating the path with the default constructor.");
			if (recycled) throw new Exception ("The path is currently in a path pool. Are you sending the path for calculation twice?");
			if (pathHandler == null) throw new Exception ("Field pathHandler is not set. Please report this bug.");
			if (GetState() > PathState.Processing) throw new Exception ("This path has already been processed. Do not request a path with the same path object twice.");
		}
		...
		public virtual void OnEnterPool () {
			if (vectorPath != null) ListPool<Vector3>.Release (vectorPath);
			if (path != null) ListPool<GraphNode>.Release (path);
			vectorPath = null;
			path = null;
		}
...
\endcode
		...
		public void Claim (Object o) {
			if (ReferenceEquals (o, null)) throw new ArgumentNullException ("o");
		...
		public void ReleaseSilent (Object o) {
			...
			if (claimed.Count == 0) {
				throw new ArgumentException ("You are releasing a path which is not claimed at all (most likely it has been pooled already). " +
					"Are you releasing the path with the same object ("+o.ToString()+") twice?");
			} else {
				throw new ArgumentException ("You are releasing a path which has not been claimed with this object ("+o.ToString()+"). " +
