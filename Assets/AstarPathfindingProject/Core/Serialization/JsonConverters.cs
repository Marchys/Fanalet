...
namespace Pathfinding.Serialization
{
	...
	public class UnityObjectConverter : JsonConverter {
		...
		public override object ReadJson ( Type objectType, Dictionary<string,object> values) {
			...
			var typename = (string)values["Type"];
			...
			if (values.ContainsKey ("GUID")) {
				var guid = (string)values["GUID"];
				
				var helpers = Object.FindObjectsOfType(typeof(UnityReferenceHelper)) as UnityReferenceHelper[];
	...
	public class GuidConverter : JsonConverter {
		public override bool CanConvert (Type type) {
			return Equals ( type, typeof(Guid) );
		}
	...
	public class MatrixConverter : JsonConverter {
		public override bool CanConvert (Type type) {
			return Equals ( type, typeof(Matrix4x4) );
		}
		...
		public override object ReadJson ( Type objectType, Dictionary<string,object> values) {
			...
			for (var i=0;i<16;i++) m[i] = Convert.ToSingle (arr.GetValue(new int[] {i}));
		...
		public override Dictionary<string,object> WriteJson (Type type, object value) {
			...
			for (var i=0;i<values.Length;i++) values[i] = m[i];
	...
	public class BoundsConverter : JsonConverter {
		public override bool CanConvert (Type type) {
			return Equals ( type, typeof(Bounds) );
		}
	...
	public class LayerMaskConverter : JsonConverter {
		public override bool CanConvert (Type type) {
			return Equals ( type, typeof(LayerMask) );
		}
	...
	{
		public override bool CanConvert (Type type) {
			return Equals ( type, typeof(Vector2) ) || Equals ( type, typeof(Vector3) )||Equals ( type, typeof(Vector4) );
		}
		
		public override object ReadJson (Type type, Dictionary<string,object> values) {
			if (Equals ( type, typeof(Vector2) )) {
				return new Vector2(CastFloat(values["x"]),CastFloat(values["y"]));
			} else if (Equals ( type, typeof(Vector3) )) {
				return new Vector3(CastFloat(values["x"]),CastFloat(values["y"]),CastFloat(values["z"]));
			} else if (Equals ( type, typeof(Vector4) )) {
	...
	public class IntKeyDictionaryConverter : JsonConverter {
        public override bool CanConvert (Type type) {
            return ( Equals (type, typeof(Dictionary<int,int>)) || Equals (type, typeof(SortedDictionary<int,int>)) );
        }
        ...
        public override object ReadJson (Type type, Dictionary<string,object> values) {
            ...
            {
                holder.Add( Convert.ToInt32(val.Key), Convert.ToInt32(val.Value) );
            }
            return holder;
        }
