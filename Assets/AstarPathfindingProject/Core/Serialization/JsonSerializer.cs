...
//using CompatFileStream = MarkerMetro.Unity.WinLegacy.IO.FileStream;
...
namespace Pathfinding.Serialization
{
	...
	public class AstarSerializer
	{
		 ...
		 */
		private static StringBuilder GetStringBuilder () { _stringBuilder.Length = 0; return _stringBuilder; }
...
#region Serialize
		...
		public void SerializeNodes () {
			...
			for (var i=0;i<graphs.Length;i++) {
				var bytes = SerializeNodeConnections (i);
...
#region Deserialize
		...
		public NavGraph[] DeserializeGraphs () {
			...
			for (var i=0;i<meta.graphs;i++) {
				...
				var tmp = data.CreateGraph(tp);//(NavGraph)System.Activator.CreateInstance(tp);
		...
		public void DeserializeNodes () {
			...
			for (var i=0;i<graphs.Length;i++) {
				...
				var entry = zip["graph"+i+"_nodes"+binaryExt];
			...
			for (var i=0;i<graphs.Length;i++) {
				...
				var entry = zip["graph"+i+"_conns"+binaryExt];
				if (entry == null) continue;
		...
		public void DeserializeExtraInfo () {
			...
			
			{
				...
				for (var i=0;i<graphs.Length;i++) {
					...
					var ctx = new GraphSerializationContext(reader, int2Node, i);
