...
namespace Pathfinding {
	...
	public class TriangleMeshNode : MeshNode {
		...
		public static void SetNavmeshHolder (int graphIndex, INavmeshHolder graph) {
			if (_navmeshHolders.Length <= graphIndex) {
				...
				for (var i=0;i<_navmeshHolders.Length;i++) gg[i] = _navmeshHolders[i];
				_navmeshHolders = gg;
			}
		...
		public override Vector3 ClosestPointOnNodeXZ (Vector3 _p) {
			var g = GetNavmeshHolder(GraphIndex);
			var tp1 = g.GetVertex(v0);
			var tp2 = g.GetVertex(v1);
			var tp3 = g.GetVertex(v2);
			...
			if ((long)(tp2.x - tp1.x) * (long)(p.z - tp1.z) - (long)(p.x - tp1.x) * (long)(tp2.z - tp1.z) > 0) {
				var f = Mathf.Clamp01 (AstarMath.NearestPointFactor (tp1, tp2, p));
				...
				var f = Mathf.Clamp01 (AstarMath.NearestPointFactor (tp2, tp3, p));
				...
				var f = Mathf.Clamp01 (AstarMath.NearestPointFactor (tp3, tp1, p));
		...
		public override bool ContainsPoint (Int3 p) {
			...
			var a = g.GetVertex(v0);
			var b = g.GetVertex(v1);
			var c = g.GetVertex(v2);
	...
	public class ConvexMeshNode : MeshNode {
		...
		public override void GetConnections (GraphNodeDelegate del) {
			if (connections == null) return;
			for (var i=0;i<connections.Length;i++) del (connections[i]);
		}
