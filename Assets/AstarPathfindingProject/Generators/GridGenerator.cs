...
\nosubgrouping
	...
	public class GridGraph : NavGraph, IUpdatableGraph
	{
		...
		public override NNInfo GetNearest (Vector3 position, NNConstraint constraint, GraphNode hint) {
			...
			var xf = position.x-0.5F;
			var zf = position.z-0.5f;
			var x = Mathf.Clamp (Mathf.RoundToInt (xf)  , 0, width-1);
			var z = Mathf.Clamp (Mathf.RoundToInt (zf)  , 0, depth-1);
			
			var nn = new NNInfo(nodes[z*width+x]);
			
			var y = inverseMatrix.MultiplyPoint3x4((Vector3)nodes[z*width+x].position).y;
		...
		public override NNInfo GetNearestForce (Vector3 position, NNConstraint constraint) {
			...
			if (constraint.Suitable (node)) {
				...
				var y = inverseMatrix.MultiplyPoint3x4((Vector3)node.position).y;
			...
			var maxDist = constraint.constrainDistance ? AstarPath.active.maxNearestNodeDistance : float.PositiveInfinity;
		...
		public virtual void SetUpOffsetsAndCosts () {
			...
			var diagonalCost = (uint)Mathf.RoundToInt (nodeSize*Mathf.Sqrt (2F)*Int3.Precision);
					...
					for (var z = zmin; z < zmax; z ++) {
						for (var x = xmin; x < xmax; x++) {
							...
							if (node.Walkable && node.Tag >= erosionFirstTag && node.Tag < erosionFirstTag + it) {
								...
								for (var i=0;i<4;i++) {
									var other = GetNodeConnection (node,i);
									if (other != null) {
										var tag = other.Tag;
				...
				for (var x = 0; x < width; x++) {
					...
					if (AstarPath.active.showSearchTree && debugData != null) {
						if (InSearchTree(node,AstarPath.active.debugPath)) {
							var nodeR = debugData.GetPathNode (node);
		...
		private List<GraphNode> GetNodesInArea (Bounds b, GraphUpdateShape shape) {
			...
			var inArea = ListPool<GraphNode>.Claim ();
		...
		public void UpdateArea (GraphUpdateObject o) {
			...
			var erosion = o.updateErosion ? erodeIterations : 0;
			
			
			var willChangeWalkability = o.updatePhysics || o.modifyWalkability;
