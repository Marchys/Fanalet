...
public class ProceduralGridMover2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (AstarMath.SqrMagnitudeXY(target.position, graph.center) > updateDistance * updateDistance)
        {
            UpdateGraph();
        }
    }
    ...
    IEnumerator UpdateGraphCoroutine()
    {
        ...
        dir.y = Mathf.Round(dir.y / graph.nodeSize) * graph.nodeSize;
        dir.z = 0;
        ...
        // direction
        var offset = new Int2(-Mathf.RoundToInt(dir.x / graph.nodeSize), -Mathf.RoundToInt(dir.y / graph.nodeSize));
